...
// ── thinking-filler voice clips ("umm / hmm") ────────────────────────────────
// Real spoken fillers in Diana's voice, generated ONCE via Live and cached on the SD card, then
// played instantly (local, no network) at the start of each turn so the wait feels like she's
// thinking. Cached files survive reboots/reflashes (SD isn't wiped).
static const char* FILLER_PHRASES[] = { "Hmm...", "Umm, let me think.", "Mmm, one moment." };
static const int   FILLER_COUNT = 3;
static int         g_fillersReady = -1;   // -1 unknown, else count of existing clips

static bool genFiller(const String& text, const char* path) {
    if (!sdOk) return false;
    // The Live API occasionally returns an empty turn; retry so the clip reliably caches.
    for (int attempt = 0; attempt < 3; ++attempt) {
        File f = SD.open(path, FILE_WRITE);
        if (!f) return false;
        size_t got = 0;
        String err;
        bool ok = GeminiLive.speak(text, Config.ttsVoiceOrDefault(), "", "", Config.apiKey.c_str(),
                                   [&](const uint8_t* pcm, size_t n) { f.write(pcm, n); got += n; },
                                   []() { return true; }, err);
        f.close();
        if (ok && got >= 4000) return true;     // got real audio
        SD.remove(path);
        delay(200);
    }
    return false;
...
static void generateFillersIfMissing() {
    if (!sdOk || !Net.isConnected()) return;
    if (!SD.exists(DIANA_DIR)) SD.mkdir(DIANA_DIR);
    int ready = 0;
    for (int i = 0; i < FILLER_COUNT; ++i) {
        char p[32]; snprintf(p, sizeof(p), "%s/fill%d.pcm", DIANA_DIR, i);
        if (SD.exists(p)) { ready++; continue; }
        UI.log("preparing her voice...", 's'); UI.tick();
        if (genFiller(FILLER_PHRASES[i], p)) ready++;
    }
    g_fillersReady = ready;
}

// Play a random cached "umm/hmm" clip (blocking, ~0.5-1s); falls back to the soft hum if none exist.
static void playThinking() {
    if (!canSpeak()) return;
    if (g_fillersReady < 0) {
        g_fillersReady = 0;
        for (int i = 0; i < FILLER_COUNT; ++i) { char p[32]; snprintf(p, sizeof(p), "%s/fill%d.pcm", DIANA_DIR, i); if (SD.exists(p)) g_fillersReady++; }
    }
    if (g_fillersReady > 0) {
        char p[32]; snprintf(p, sizeof(p), "%s/fill%d.pcm", DIANA_DIR, (int)random(g_fillersReady));
        Audio.playPcmFile(p, TTS_RATE, playbackTick);
    } else {
        Audio.thinking();   // fallback hum if clips aren't generated yet
    }
}
...
static void welcomeProtocol() {
    ...
    // Wake-up call: "Hello. <time greeting>. I am Diana, D-I-0336-7. Happy to help you."
    // (no user name for now, per request)
    (void)name;
    String greet = "Hello.";
...
void loop() {
    ...
    handleKeys();
    serviceTimers();
    if (awake && Net.isConnected()) GeminiLive.keepAlive();   // keep the voice session warm between turns
